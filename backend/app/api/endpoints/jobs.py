...
def download_job_results(
    ...
    """
    Download job results as a formatted text report.
    Includes job metadata, execution logs, and pipeline step details.
    """
    ...
    lines = []
    lines.append("=" * 60)
    lines.append(f"  HIVE — Job Execution Report")
    lines.append("=" * 60)
    lines.append("")
    lines.append(f"Job ID:          {job.id}")
    lines.append(f"Status:          {job.status}")
    lines.append(f"Created:         {job.created_at}")
    lines.append(f"Started:         {job.started_at or 'N/A'}")
    lines.append(f"Finished:        {job.finished_at or 'N/A'}")

    if job.started_at and job.finished_at:
        duration = (job.finished_at - job.started_at).total_seconds()
        lines.append(f"Duration:        {duration:.1f}s")

    lines.append(f"CPU Requested:   {job.cpu_req} core(s)")
    lines.append(f"RAM Requested:   {job.ram_req} GB")
    lines.append(f"Cost:            {job.cost:.4f} credits")

    if job.build_command:
        lines.append(f"Build Command:   {job.build_command}")
    if job.run_command:
        lines.append(f"Run Command:     {job.run_command}")

    # Pipeline steps
    steps = crud.crud_job.get_pipeline_steps(db, job_id=job_id)
    if steps:
        lines.append("")
        lines.append("-" * 60)
        ...
        for step in steps:
            ...
            if step.started_at and step.finished_at:
                d = (step.finished_at - step.started_at).total_seconds()
                lines.append(f"      Duration: {d:.1f}s")
            if step.log:
                lines.append(f"      Log:")
                for log_line in step.log.splitlines():
        ...
        if logs:
            ...
            lines.append("")
            for log in logs:
                lines.append(log.content or "")

    lines.append("")
    lines.append("=" * 60)
    lines.append(f"  Report generated by Hive")
    lines.append("=" * 60)
    lines.append("")
    ...
    filename = f"hive_job_{job_id}_results.txt"
